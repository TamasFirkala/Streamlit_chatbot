...
# Import Pinecone with error handling for both versions
try:
    # Try importing the newer V2 version first
...
# Get secrets/environment variables in a consistent way for both local and cloud environments
...
# Try to load dotenv only in local environment
try:
...
with tab1:
    try:
        ...
        pinecone_env = get_env_variable("pinecone_environment")
        ...
        # Initialize Pinecone based on available version
        if PINECONE_V2:
